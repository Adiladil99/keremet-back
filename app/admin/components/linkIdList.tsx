...
const Edit = (props: BasePropertyProps) => {
  ...
  return (
    <Box marginBottom="xxl">
      {record?.populated?.parentId?.params.id && <a className='reference-value__StyledLink-sc-1ujousf-0 hjMVLh' href={"/admin/resources/categories/records/"+record?.populated?.parentId?.params.id+"/show"}>{data?.ru}</a>}
    </Box> 
  )
}
